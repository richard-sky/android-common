...
public class FlowLayout extends ViewGroup {
    ...
    private final List<Integer> lineNumList = new ArrayList<>();
    ...
    public FlowLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        lineSpacing = DensityUtilKt.dp2px(10, context);
    }
    ...
    private void compress() {
        ...
        for (int i = 0; i < count; i++) {
            compressSpaces[i] = Math.min(spaces[i], usefulWidth);
        }
                ...
                if (blankCount >= 0) {
                    if (blankCount > 0) {
                        ...
                        for (int j = start; j < end; j++) {
                            ...
                            BlankView blank = new BlankView(getContext());
