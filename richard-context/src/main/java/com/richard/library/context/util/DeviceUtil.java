...
public final class DeviceUtil {
    ...
    public static String getUserAgent() {
        return String.format("Android;%s;%s;%s-%s", getOSVersionName(), getAppVersionName(), getVendor(), getDevice());
    }
     ...
     * @param alpha （0-1）
     */
    public static void setScreenAlpha(Context context, float alpha) {
    ...
    /**
     * 设置当前界面的屏幕亮度（退出该界面时恢复原屏幕亮度）
     *
     * @param context    context
     * @param brightness (1-255)
     */
    public static void setCurrentScreenLight(Activity context, int brightness) {
     ...
     */
    public static boolean toDial(Context context, final String phoneNumber) {
     ...
     * @param url 网页地址
     */
    public static void toWebBrowser(String url) {
        ...
        try {
            ...
            Toast.makeText(AppContext.get(), "未安装浏览器", Toast.LENGTH_SHORT).show();
        }
    }
     ...
     * 跳转到指定app
     *
     * @param packageName 必填 其他app包名
     * @param defaultTip  选填 默认提示
     */
    public static void toApp(String packageName, String defaultTip) {
        try {
            Intent intent = AppContext.get().getPackageManager().getLaunchIntentForPackage(packageName);
            AppContext.get().startActivity(intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK));
        } catch (Throwable t) {
            Toast.makeText(AppContext.get(),
                    TextUtils.isEmpty(defaultTip) ? "暂未安装该应用" : defaultTip, Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private static String getMacAddressByWifiInfo() {
        try {
            final WifiManager wifi = (WifiManager) AppContext.get()
    ...
    private static String getMacAddressByFile() {
        CommandResult result = DeviceUtil.execCmd(new String[]{"getprop wifi.interface"}, null
                , false, true);
        if (result.result == 0) {
            String name = result.successMsg;
            if (name != null) {
                result = DeviceUtil.execCmd(new String[]{"cat /sys/class/net/" + name + "/address"}
                        , null, false, true);
                if (result.result == 0) {
    ...
    public static boolean isEmulator() {
        ...
        boolean checkDial = intent.resolveActivity(AppContext.get().getPackageManager()) == null;
...
//        if (checkDebuggerConnected) return true;
    ...
    public static String getUniqueDeviceId(String prefix, boolean useCache) {
        ...
        if (udid == null) {
            synchronized (DeviceUtil.class) {
                if (udid == null) {
                    final String id = SPUtil.getString(KEY_UDID, null);
                    if (id != null) {
                        udid = id;
                        return udid;
                    }
    ...
    public static boolean isSameDevice(final String uniqueDeviceId) {
        ...
        final String cachedId = SPUtil.getString(KEY_UDID, null);
     ...
     * @return 字节（byte）
     */
    public static long getExternalRemainingSpace() {
        String state = Environment.getExternalStorageState();
        if (!Environment.MEDIA_MOUNTED.equals(state)) {
            return 0;
        }
     ...
     * @return 字节(byte)
     */
    public static long getSystemRemainingSpace() {
        File root = Environment.getRootDirectory();
        StatFs sf = new StatFs(root.getPath());
        long blockSize = sf.getBlockSizeLong();
        long availCount = sf.getAvailableBlocksLong();
    ...
    public static long getRAMTotalSpace() {
        ActivityManager activityManager = (android.app.ActivityManager) AppContext.get().getSystemService(Context.ACTIVITY_SERVICE);
     ...
     * @return 字节(byte)
     */
    public static long getRAMRemainingSpace() {
        ActivityManager activityManager = (android.app.ActivityManager) AppContext.get().getSystemService(Context.ACTIVITY_SERVICE);
        ActivityManager.MemoryInfo info = new ActivityManager.MemoryInfo();
        activityManager.getMemoryInfo(info);
    ...
    public static CommandResult execCmd(final String[] commands,
        ...
        try {
            ...
            errorMsg.append(LINE_SEP).append(e.getMessage());
        } finally {
            try {
                if (os != null) {
                    os.close();
