...
import com.richard.library.bluetooth.core.bluetooth.BleBluetooth;
import com.richard.library.bluetooth.core.bluetooth.MultipleBluetoothController;
import com.richard.library.bluetooth.core.bluetooth.SplitWriter;
import com.richard.library.bluetooth.core.callback.BleGattCallback;
import com.richard.library.bluetooth.core.callback.BleIndicateCallback;
import com.richard.library.bluetooth.core.callback.BleMtuChangedCallback;
import com.richard.library.bluetooth.core.callback.BleNotifyCallback;
import com.richard.library.bluetooth.core.callback.BleReadCallback;
import com.richard.library.bluetooth.core.callback.BleRssiCallback;
import com.richard.library.bluetooth.core.callback.BleScanAndConnectCallback;
import com.richard.library.bluetooth.core.callback.BleScanCallback;
import com.richard.library.bluetooth.core.callback.BleWriteCallback;
...
import com.richard.library.bluetooth.core.data.BleScanState;
import com.richard.library.bluetooth.core.exception.BleException;
import com.richard.library.bluetooth.core.exception.OtherException;
import com.richard.library.bluetooth.core.scan.BleScanRuleConfig;
import com.richard.library.bluetooth.core.scan.BleScanner;
import com.richard.library.bluetooth.core.utils.BleLog;
...
public class BleManager {
     ...
     *
     * @param mac              蓝牙mac地址
     * @param isAutoDisconnect 写入完成或发生异常后是否自动断开连接gatt
     * @return BluetoothGatt
     */
    public BluetoothGatt connectAndWrite(String mac, byte[] data, boolean isAutoDisconnect, ConnectAndWriteCallback callback) {
        ...
        return this.connectAndWrite(bleDevice, BluetoothGattCharacteristic.PROPERTY_WRITE, data, isAutoDisconnect, callback);
    }
     ...
     * @return BluetoothGatt
     */
    public BluetoothGatt connectAndWrite(BleDevice bleDevice, byte[] data, boolean isAutoDisconnect, ConnectAndWriteCallback callback) {
        return this.connectAndWrite(bleDevice, BluetoothGattCharacteristic.PROPERTY_WRITE, data, isAutoDisconnect, callback);
    }
     ...
     * @return BluetoothGatt
     */
    public BluetoothGatt connectAndWrite(BleDevice bleDevice, int propertyType, byte[] data, boolean isAutoDisconnect, ConnectAndWriteCallback callback) {
        if (BleManager.getInstance().isConnected(bleDevice)) {
            BluetoothGatt gatt = BleManager.getInstance().getBluetoothGatt(bleDevice);
            if (gatt != null) {
                this.startWrite(bleDevice, gatt, propertyType, data, isAutoDisconnect, callback);
                return gatt;
            }
        ...
        return BleManager.getInstance().connect(bleDevice, new BleGattCallback() {
            ...
            public void onConnectFail(BleDevice bleDevice, BleException exception) {
                callback.onWriteFailure(bleDevice, exception);
            }

            @Override
            public void onConnectSuccess(BleDevice bleDevice, BluetoothGatt gatt, int status) {
                startWrite(bleDevice, gatt, propertyType, data, isAutoDisconnect, callback);
            }

            @Override
            public void onDisConnected(boolean isActiveDisConnected, BleDevice device, BluetoothGatt gatt, int status) {
                if (status != 0) {
                    callback.onWriteFailure(device, new OtherException("已断开连接"));
                }
            }
        });
    }
     ...
     * @param callback         回调
     */
    private void startWrite(BleDevice bleDevice, BluetoothGatt gatt, int propertyType, byte[] data, boolean isAutoDisconnect, ConnectAndWriteCallback callback) {
        BluetoothGattCharacteristic writeChannel = BleUtil.getGattCharacteristic(gatt, propertyType);
        if (writeChannel == null) {
            callback.onWriteFailure(bleDevice, new OtherException("不支持写入数据"));
            return;
        }

        BleManager.getInstance().write(
                bleDevice
                , writeChannel.getService().getUuid().toString()
                , writeChannel.getUuid().toString()
                , data
                , new BleWriteCallback() {
                    @Override
                    @SuppressWarnings("all")
                    public void onWriteSuccess(int current, int total, byte[] justWrite) {
                        if (isAutoDisconnect && current >= total) {
                            gatt.disconnect();
                        }
                        callback.onWriteSuccess(bleDevice, current, total, justWrite);
                    }
    ...
    public BluetoothGatt connect(BleDevice bleDevice, BleGattCallback bleGattCallback) {
        ...
        if (!isBlueEnable()) {
            BleLog.e("未开启蓝牙");
            bleGattCallback.onConnectFail(bleDevice, new OtherException("未开启蓝牙"));
            return null;
        }
        ...
        if (bleDevice == null || bleDevice.getDevice() == null) {
            bleGattCallback.onConnectFail(bleDevice, new OtherException("未找到蓝牙设备"));
        } else {
    ...
    public boolean isConnected(String mac) {
        return getBleDevice(mac) != null;
    }

    public void disconnect(String mac) {
        BleDevice bleDevice = getBleDevice(mac);
        if (bleDevice != null && multipleBluetoothController != null) {
